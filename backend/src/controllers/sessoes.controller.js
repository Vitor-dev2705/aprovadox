...
exports.create = async (req, res) => {
  try {
    const { materia_id, assunto_id, tecnica, duracao_minutos, data_inicio, data_fim, notas } = req.body;
    const result = await pool.query(
      `INSERT INTO sessoes_estudo (user_id, materia_id, assunto_id, tecnica, duracao_minutos, data_inicio, data_fim, notas)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [req.userId, materia_id, assunto_id, tecnica, duracao_minutos, data_inicio, data_fim, notas]
    );
    ...
    // Schedule reviews (24h, 7d, 30d, 90d)
    if (assunto_id) {
      const intervals = ['1 day', '7 days', '30 days', '90 days'];
      const tipos = ['24h', '7d', '30d', '90d'];
      for (let i = 0; i < intervals.length; i++) {
        await pool.query(
          `INSERT INTO revisoes (user_id, materia_id, assunto_id, sessao_id, tipo, data_revisao)
           VALUES ($1, $2, $3, $4, $5, CURRENT_DATE + INTERVAL '${intervals[i]}')`,
          [req.userId, materia_id, assunto_id, sessao.id, tipos[i]]
        );
      }
    }
